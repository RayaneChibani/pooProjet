...
robot::robot(terrain& terrain): d_terrain{terrain}, x{terrain.getDepart().first}, y{terrain.getDepart().second}, direction{EST}, d_observateurs{}
{}

   void robot::ajouterObservateur(std::unique_ptr <observateurRobot> observateur) {
    ...
    const std::vector<std::unique_ptr<observateurRobot>>& robot::getObservateurs() const {
        return d_observateurs;  // Retourne une référence constante au vecteur
    }
